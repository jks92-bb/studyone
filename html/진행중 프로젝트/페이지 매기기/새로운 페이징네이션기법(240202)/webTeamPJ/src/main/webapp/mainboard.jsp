...
<body>
	<div>
		<img class="logoImage"
			...
			alt="로고 이미지" onclick="refreshPage()" >
    ...
    <form action="search.doPosting" method="get">
        <img class="searchengine"
            ...
            alt="검색엔진 이미지">
        <input class="searching" type="text" name="searchKeyword" />
		...
		<form action="viewBoard.jsp" >
			<input type="s014" value="게시판2" name="2" style="font-size: 30px;"/>
		</form>
		<form action="viewBoard.jsp" >
			<input type="submit" value="게시판3" name="3" style="font-size: 30px"/>
		</form>
		<form action="viewPL.doPosting">
			<input type="submit" value="글쓰기" name="3" style="font-size: 30px"/>
		</form>
	</div>
    ...
    %>
                <li>
                    <a href="detailPost.jsp" name="<%= boardInfo.get("idx")%>"><%= boardInfo.get("tag") %> : <%= boardInfo.get("title") %></a>
                </li>
    <%
            }
        } else {
