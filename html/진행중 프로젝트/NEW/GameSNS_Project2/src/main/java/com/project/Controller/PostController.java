...
import com.project.Command.pagination; // 수정
...
public class PostController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
     ...
     // Pagination 객체 생성
        pagination paginator = new pagination(totalItems, requestedPage, itemsPerPage);

        // 글 목록 가져오기
        List<PostDTO> posts = postDAO.getPostsByPage(keyword, paginator.getOffset(), paginator.getItemsPerPage());

        // 페이지 정보를 request에 저장
        //request.setAttribute("pagination", paginator);
        request.setAttribute("pagination", paginator.getNowPage());
